...
import tkinter.font
import urllib.parse
from chapter7 import style, Rect, DrawRect, DrawLine, BLOCK_ELEMENTS, DrawOutline, get_font
from pre_browser import cascade_priority, print_tree, tree_to_list, DescendantSelector
...
class InputLayout:
    def __init__(self, node, parent, previous):
        ...
        if self.node.tag == "input" and self.node.attributes.get("type", "text") == "checkbox":
            ...
            self.type = "button"
        # notetake
        elif self.node.tag == "input" and not isinstance(self.previous, InputLayout) and self.previous is not None and "Hidden" in self.previous.word:
            self.type = "hidden"
        else:
            self.type = "text"
