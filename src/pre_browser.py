...
class BlockLayout:
    ...
    def layout(self):
        ...
        if mode == "block":
            ...
            # if group:
            #     groups.append(group)
            # self.children = [BlockLayout(group, self, previous)
            #                  for group in groups]
    ...
    def paint(self):
        ...
        #             and "class" in self.nodes[0].attributes and self.nodes[0].attributes["class"] == "links":
        #         x2, y2 = self.x + self.width, self.y + self.height
        ...
        #     # Bullets
        #     if isinstance(self.nodes[0], Element) and self.nodes[0].tag == "li":
        #         bullet_x = self.x - HSTEP - 2  # Adjust left value
        #         bullet_y = self.y + self.height_of_first_line / 2 + 8  # Adjust top value
        #         rect = DrawRect(bullet_x, bullet_y, bullet_x +
        #                         4, bullet_y + 4, "black")
        #         cmds.append(rect)
        ...
        #         and "class" in self.node.attributes and self.node.attributes["class"] == "links":
...
def style(node, rules):
    ...
    if isinstance(node, Element) and "style" in node.attributes:
        ...
        for property, value in pairs.items():
            node.style[property] = value
    if isinstance(node, Element) and node.tag == "input" and node.attributes.get("type", "").lower() == "hidden":
...
if __name__ == "__main__":
    ...
    # url = URL(wbemocks.socket.serve(html))
    this_browser = Browser()
    this_browser.load(url)
    pairs = CSSParser("font: normal normal 32px Menlo; color:yellow;").body()
